...
class AppWindow(Gtk.ApplicationWindow):
	def __init__(self,*args,**kwargs):
		...
		buttonwin=Gtk.Button("Popup Memo")
		...
		cleanbtn = Gtk.Button("*CLEAN ALL LINES*You can't recover deleted lines!")
	...
	def newwin(self, widget):
		try:
			try:
				Popen("dedit-jp", shell=False)
			except:
				Popen("/usr/bin/dedit-jp", shell=False)
		except:
			print("ERROR")
		...
		for w in args:
			...
			try:
				try:
					...
					with open(w, 'r', encoding='shift_jis') as f:
						data=f.read()
						self.Text.set_text(w)
						self.Text1.insert(start2,data)
						print("Keisung")
